...
(function () {
  ...
  urlDownloadBtn.addEventListener("click", () => {
    ...
    if (flags.length === 0) {
      ...
      flags.forEach((f) => {
        ...
        const lines = ctx.doc.splitTextToSize("• " + f, ctx.pageWidth - ctx.marginX * 2);
    ...
    (lastUrlResult.images_scanned || []).forEach((img, idx) => {
      ...
      const urlLines = ctx.doc.splitTextToSize((idx + 1) + ". " + (img.image_url || ""), ctx.pageWidth - ctx.marginX * 2);
  ...
  function pdfKeyValue(ctx, key, value) {
    ...
    ctx.doc.text(String(key).toUpperCase(), ctx.marginX, ctx.y);
    ctx.doc.setFont("helvetica", "normal");
    ctx.doc.setFontSize(11);
    ctx.doc.setTextColor(30, 30, 40);
    const lines = ctx.doc.splitTextToSize(String(value), ctx.pageWidth - ctx.marginX * 2 - 130);
  ...
  function pdfSectionTitle(ctx, text) {
    ...
    ctx.doc.text(text, ctx.marginX, ctx.y);
    ctx.y += 16;
  }
  ...
  downloadBtn.addEventListener("click", () => {
    ...
    const explLines = ctx.doc.splitTextToSize(
      lastResult.explanation || "No further explanation was provided by the model.",
  ...
  batchDownloadBtn.addEventListener("click", () => {
    ...
    lastBatchResults.forEach((r, idx) => {
      ...
      ctx.doc.setFontSize(10.5);
      ctx.doc.setTextColor(30, 30, 40);
      ctx.doc.text((idx + 1) + ". " + (r.filename || "unknown"), ctx.marginX, ctx.y);
      ctx.y += 14;
  ...
  videoDownloadBtn.addEventListener("click", () => {
    ...
    ctx.doc.setTextColor(50, 50, 60);
    const explLines = ctx.doc.splitTextToSize(
      lastVideoResult.explanation || "No further explanation was provided by the model.",
      ctx.pageWidth - ctx.marginX * 2
    );
    ctx.doc.text(explLines, ctx.marginX, ctx.y);
    ...
    (lastVideoResult.frame_results || []).forEach((f) => {
      ...
      ctx.doc.setTextColor(50, 50, 60);
      const line = f.error
        ? "Frame #" + f.frame_index + " (" + ts + "): Error — " + f.error
